...
public class ReflectionUtils {
    ...
    public static <R> R getValue(final Object instance, final String fieldName) {
        List<Field> fieldList = getFieldList(instance, field -> field.getName().equals(fieldName));
        try {
            for (Field field : fieldList) {
                return (R) getFieldValue(field, instance);
            }
    ...
    public static List<Field> getFieldList(final Object instance, Predicate<Field> predicate) {
        ...
        try {
            ...
            while (instanceClass != Object.class) {
                fields.addAll(Arrays.stream(instanceClass.getDeclaredFields()).filter(predicate).toList());
    ...
    public static List<Field> getFieldList(Class<?> clazz, Predicate<Field> predicate) {
        List<Field> fields = new ArrayList<>();
        try {
            Objects.requireNonNull(clazz);
            Objects.requireNonNull(predicate);
            while (clazz != Object.class) {
                fields.addAll(Arrays.stream(clazz.getDeclaredFields()).filter(predicate).toList());
     ...
     * @param args:       arguments pass to method
     * @return Object
     */
    @SuppressWarnings("unchecked")
